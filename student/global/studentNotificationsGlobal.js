...
function createNotificationCard(notification, isRead) {
    ...
    card.addEventListener("click", async function () {
        ...
        if ((type === "Posts" || type === "Post Comments") && notification.Post_ID) {
            ...
            window.location.href = `https://courses.writerscentre.com.au/course-details/content/${lessonUid}?eid=${myEidFromLesson}&classIdFromUrl=${classIdUrl}&className=${classNameUrl}&classUid=${classUniqueId}&currentClassID=${classIdUrl}&currentClassName=${classNameUrl}&currentClassUniqueID=${classUniqueId}&submissionPostIs=${notification.Notification_Type != 'Submission Comments' ? notification.Submissions_ID : commentIdForNoti}${assessmentType === "File Submission" ? `&subUID=${subUID}&commentScrollId=${commentScrollID}` : ""}`;
        } else {
            const myEidFromCourse = await getEnrolmentIdsByCourseUid(classIdUrl);
