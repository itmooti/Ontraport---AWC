...
async function loadPosts(filter = "all") {
  if (!classIdForForumChat) {
    ...
    if (!fetchedClassID) {
      ...
      $("#forumContainer").html(
        "<div class='text-red-500'>Class ID not found</div>"
      );
      return;
    }
...
$(document).ready(async function () {
  ...
  $("#submit-post").on("click", function (event) {
    ...
    try {
      console.log("[Forum] Submit post clicked", {
        classId: classIdForForumChat || window.classID,
    ...
    try {
      console.log("[Forum] Mentions parsed", {
        count: mentionedIds.length,
        mentionedIds,
      });
    } catch (_) {}
    let payload = {
    ...
    if (fileInput.files && fileInput.files[0]) {
      ...
      try {
        console.log("[Forum] File selected for post", uploadedFileInfo);
      } catch (_) {}
    }
    ...
    function submitNewPost(finalPayload) {
      ...
      try {
        console.log("[Forum] Submitting post payload", {
          ...
          mentions: (finalPayload.Mentions || []).map((m) => m.id),
        });
      } catch (_) {}
      ForumAPI.createPost(finalPayload)
        .then((created) => {
          ...
          try {
            (async function createPostAlerts() {
              ...
              console.groupCollapsed &&
                console.groupCollapsed(
                  "[ForumAlerts] Begin alert creation for post",
                  created?.id
                );
              try {
                console.log("[ForumAlerts] Using class ID", clsId);
              } catch (_) {}
              ...
              const [resClasses, resEnrol, resTeacher] = await Promise.all([
                fetch(graphqlApiEndpoint, {
                  ...
                  r.ok ? r.json() : Promise.reject("getClasses query failed")
                ),
                fetch(graphqlApiEndpoint, {
                  method: "POST",
                  headers: {
                  ...
                  r.ok
                    ? r.json()
                    : Promise.reject("calcEnrolments query failed")
                ),
                fetch(graphqlApiEndpoint, {
                  method: "POST",
                  headers: {
                  ...
                  r.ok ? r.json() : Promise.reject("calcClasses query failed")
                ),
              ]);
              try {
                console.log("[ForumAlerts] Roster responses", {
                  resClasses,
                  resEnrol,
                  resTeacher,
                });
              ...
              const classes = Array.isArray(resClasses?.data?.getClasses)
                ...
                : [];
              let classUid = undefined;
              let className = undefined;
              let courseUid = undefined;
              ...
              try {
                console.log("[ForumAlerts] Roster extracted", {
                  fromClasses: idsFromClasses,
                  fromEnrol: idsFromEnrol,
                  teacherIds,
                  adminIds,
                });
              } catch (_) {}
              ...
              ids = normalize(ids).filter((n) =>
                seen.has(n) ? false : (seen.add(n), true)
              );
              try {
                console.log("[ForumAlerts] Student IDs parsed", {
                  count: ids.length,
                  ids,
                });
              } catch (_) {}
              ...
              try {
                console.log("[ForumAlerts] Audience computed", {
                  total: ids.length,
                  excludedAuthorId: authorId,
              ...
              try {
                console.log("[ForumAlerts] Alert content prepared", {
              ...
              if (window.AWC && typeof window.AWC.createAlerts === "function") {
                try {
                  ...
                  try {
                    console.log("[ForumAlerts] Alerts creation result", result);
                  } catch (_) {}
                } catch (e) {
                  console.error(
                    "Failed to create alerts via AWC.createAlerts",
                    e
                  );
                }
              } else {
                try {
                  console.warn(
                    "[ForumAlerts] window.AWC.createAlerts not available"
                  );
                } catch (_) {}
              }
          ...
          try {
            console.log(
              "[Forum] Updating has__new__notification for mentions",
              mentionIds
            );
          } catch (_) {}
          return Promise.all(
        ...
        .then((data) => {
          ...
          try {
            console.log("[Forum] Refreshing post after creation", {
              postId: data?.id,
            });
          } catch (_) {}
        ...
        .then((post) => {
          ...
          $(fileInput).val("");
          try {
            console.log("[Forum] Post rendered and input reset", {
              postId: post?.id,
            });
          } catch (_) {}
        })
    ...
    if (fileInput.files && fileInput.files[0]) {
      ...
      try {
        console.log("[Forum] Processing file upload for post", {
      ...
      try {
        console.log("[Forum] No file attached, submitting post now");
      } catch (_) {}
      ...
      .then((created) => {
        ...
        try {
          (async function createCommentAlerts() {
            ...
            const [resClasses, resEnrol, resTeacher, resParent, resMe] =
              await Promise.all([
                fetch(graphqlApiEndpoint, {
                  method: "POST",
                  headers: {
                ...
                fetch(graphqlApiEndpoint, {
                  ...
                  body: JSON.stringify({
                    query: parentType === "post" ? qParentPost : qParentComment,
                    variables: { id: parentId },
                  }),
                }).then((r) =>
                  r.ok ? r.json() : Promise.reject("parent author query failed")
                ),
                fetch(graphqlApiEndpoint, {
                  method: "POST",
                  headers: {
                  ...
                  body: JSON.stringify({
                    query: qMe,
                    variables: { id: Number(visitorContactID) },
                  }),
                }).then((r) =>
                  r.ok ? r.json() : Promise.reject("me query failed")
                ),
              ]);
            ...
            if (parentType === "post") {
              parentAuthorId =
                Number(resParent?.data?.getForumPosts?.[0]?.author_id ?? 0) ||
                null;
            } else {
              parentAuthorId =
                Number(
                  resParent?.data?.getForumComments?.[0]?.author_id ?? 0
                ) || null;
            }
