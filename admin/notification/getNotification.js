...
async function initializeSocketForContainer(containerType, limit) {
    const isBody = containerType === "body";
    const container = document.getElementById(
        isBody ? "parentNotificationTemplatesInBody" : "secondaryNotificationContainer"
    );
    const loaderId = isBody ? "socketLoader" : "socketLoadersec";
        ...
        socket.send(
            JSON.stringify({
                ...
                payload: {
                    ...
                    variables: { class_id: classIds, offset: 0, limit: limit }
                }
            })
        );
    };
    ...
    socket.onmessage = (event) => {
        ...
        notificationData.forEach((notification) => {
            if (!displayedNotifications.has(Number(notification.ID))) {
                const isRead = readAnnouncements.has(Number(notification.ID));
                const card = createNotificationCard(notification, isRead);
                container.prepend(card);
                if (isBody) cardMap.set(notification.ID, { original: card });
                else cardMap.set(notification.ID, { clone: card });
            }
        });
    ...
    socket.onclose = () => {
        ...
        setTimeout(() => {
            if (!document.hidden) initializeSocketForContainer(containerType, limit);
        }, 28000);
    };
}

document.addEventListener("DOMContentLoaded", () => {
    initializeSocketForContainer("body", 5000);  // Full feed
    initializeSocketForContainer("nav", 50);     // Lightweight nav panel
});
  ...
  `;
    card.addEventListener("click", async function () {
        ...
        if ((type === "Posts" || type === "Post Comments") && notification.Post_ID) {
            ...
            window.location.href = `https://courses.writerscentre.com.au/course-details/content/${lessonUid}?submissionPostIs=${notification.Notification_Type === 'Submissions' ? notification.Submissions_ID : commentIdForNoti}${assessmentType === "File Submission" ? `&subUID=${subUID}&commentScrollId=${commentScrollID}` : ""}&classIdFromUrl=${classIdUrl}&className=${classNameUrl}&classUid=${classIDForADmin}&currentClassID=${classIdUrl}&currentClassName=${classNameUrl}`
        } else {
