        ...
        //     function renderPopover(showUnlink) {
        //         popoverForSelect.innerHTML = ''; 

        //         [...baseButtons, showUnlink ? unlinkButton : linkButton].forEach(({ action, icon, classes = '' }) => {
        ...
        //                 anchor.setAttribute('style', 'color: #007C8F !important; text-decoration: underline !important;');
        //                 range.deleteContents();
        ...
        //             const linkTag = ancestor.closest('a');
        //             if (linkTag) {
        //                 const parent = linkTag.parentNode;
        //                 while (linkTag.firstChild) parent.insertBefore(linkTag.firstChild, linkTag);
        //                 parent.removeChild(linkTag);
        //             }
        //         }
...
document.addEventListener('DOMContentLoaded', function() {
    (function () {
            ...
            let toolbars = new Map(); // store toolbar reference for each editable
            ...
            document.addEventListener('mouseup', () => {
                setTimeout(() => {
                    ...
                    if (selectedText.length > 0 && selection.rangeCount > 0) {
                        ...
                        saveSelection();
                        renderPopover();
                        renderToolbar(toolbars.get(mentionable)); // refresh toolbar for this mentionable
            ...
            async function actionHandler(action) {
                ...
                setTimeout(() => {
                    saveSelection(); // update isLink detection
                    renderPopover(); // re-render popover
                    if (activeEditable && toolbars.has(activeEditable)) {
                        renderToolbar(toolbars.get(activeEditable)); // re-render toolbar
                    }
                    hidePopover();
...
mentionables.forEach((editableArea) => {
    ...
    // Find the closest containerForToolbar ABOVE in the DOM
