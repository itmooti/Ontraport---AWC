...
function computeDueDateFromAssessment(startDate, dueEndOfWeekValue) {
    ...
    let additionalWeeks = dueEndOfWeekValue - 1;
    let totalDays = daysUntilSunday + additionalWeeks * 7;
...
function computeDueDateFromCustomization(startDate, daysOffset) {
    ...
    firstSunday.setDate(startDate.getDate() + daysUntilSunday);
    firstSunday.setHours(23, 59, 0, 0);
    let dueDate = new Date(firstSunday);
    dueDate.setDate(firstSunday.getDate() + parseInt(daysOffset, 10));
    dueDate.setHours(23, 59, 0, 0);
...
function getStartDate() {
    ...
    return new Date(rawStartDate.getUTCFullYear(), rawStartDate.getUTCMonth(), rawStartDate.getUTCDate());
}
    ...
    try {
        ...
        if (searchQuery) {
            ...
            lessons = lessons.filter(lesson =>
                (lesson.AssessmentsName && lesson.AssessmentsName.toLowerCase().includes(q)) ||
                (lesson.Module_Module_Name && lesson.Module_Module_Name.toLowerCase().includes(q)) ||
        ...
        lessons.forEach(async (lesson) => {
            ...
            assignmentDiv.className = "p-4 bg-[#fff] justify-between items-center flex assignmentsWrapper";
            assignmentDiv.setAttribute("data-assignment-name", lesson.AssessmentsName);
            ...
            assignmentDiv.href = (lesson.Assessments_AWC_Teacher_s_Portal_Class_Assignment_Details_Page_URL || "#") + "?duedate=" + encodeURIComponent(dueDate ? formatDueString(dueDate) : "No Due Date")?classNameAdmin=${classNameAdmin}?classUIDAdmin=${classUIDAdmin};
...
// Debounce the search input to avoid rapid duplicate calls.
...
document.getElementById("sortLatest").addEventListener("click", function () {
    ...
    document.getElementById("sortOldest").classList.add("faintButton", "text-dark");
    document.getElementById("sortOldest").classList.remove("primaryButton", "text-white");
    loadAssignments();
});
...
document.getElementById("sortOldest").addEventListener("click", function () {
    ...
    this.classList.add("primaryButton", "text-white");
    this.classList.remove("faintButton", "text-dark");
    document.getElementById("sortLatest").classList.add("faintButton", "text-dark");
    document.getElementById("sortLatest").classList.remove("primaryButton", "text-white");
    loadAssignments();
});
