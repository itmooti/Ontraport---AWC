...
`;
    ...
    function computeAssessmentDueDateFromCustomization(customization, classStartDate) {
        // 1. If Specific_Date is present, use it (convert from Unix timestamp)
        if (customization.Specific_Date !== null && customization.Specific_Date !== undefined) {
            return new Date(customization.Specific_Date * 1000);
        }
        // 2. Calculate module open date
        ...
        // 3. If Days_to_Offset is present, use it
        if (customization.Days_to_Offset !== null && customization.Days_to_Offset !== undefined) {
            let dueDate = new Date(moduleOpenDate);
            dueDate.setDate(dueDate.getDate() + parseInt(customization.Days_to_Offset, 10));
            return adjustToNextSunday(dueDate);
        }
        // 4. Else if due_days_after_module_open_date is present, use it
        if (customization.Lesson_To_Modify_Due_days_after_module_open_date !== null && customization.Lesson_To_Modify_Due_days_after_module_open_date !== undefined) {
            let dueDate = new Date(moduleOpenDate);
            dueDate.setDate(dueDate.getDate() + parseInt(customization.Lesson_To_Modify_Due_days_after_module_open_date, 10));
            return adjustToNextSunday(dueDate);
        }
        // 5. Else, use classStartDate
        return adjustToNextSunday(new Date(classStartDate));
    }
        ...
        try {
            ...
            lessons.forEach(async (lesson) => {
                ...
                if (customizations && customizations.length > 0) {
                    ...
                    dueDate = computeAssessmentDueDateFromCustomization(latestCustomization, startDate);
                } else {
