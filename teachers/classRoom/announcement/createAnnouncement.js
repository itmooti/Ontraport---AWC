 const DEFAULT_AVATAR =
    ...
    document.getElementById("confirmSchedule").addEventListener("click", async () => {
        ...
        const userMentionsFromMentionable = gatherMentionsFromElementt(document.querySelector('#confirmSchedule').closest('#announcementForm').querySelector(".mentionable"));
        if (!selectedOption) {
            return;
        }
    ...
    async function postScheduledAnnouncementNow(id) {
        ...
        announcementForms.classList.add("opacity-50", "pointer-events-none", "cursor-not-allowed");
        const announcementEl = document.querySelector(
            `[data-announcement-template-id="${id}"]`
        );
        if (announcementEl) announcementEl.style.opacity = 0.5;
